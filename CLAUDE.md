...
- **The season in progress is judged on `games` PLUS `scheduled_games`, never on `games` alone.** Every count check was written against finished seasons, and on opening night 2026 the first Thursday result made the live season read `1 != 272` and stopped the daily publish for two days (runs of 2026-09-10 and -11). `in_progress` — fixtures still scheduled, or the calendar's current season — switches the rule to `played + scheduled == 272`, per team `== 17`, and the playoff count to an upper bound until the Super Bowl is in. A dropped fixture still fails (`2 + 269 != 272`); `backend/tests/test_integrity.py` pins both directions. The NBA sibling already tolerates a partial `max_season`; do not port its `n < 60` floor here, because the NFL's 272 is exact on every day of the season.
